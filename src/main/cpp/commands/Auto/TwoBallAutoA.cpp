...
TwoBallAutoA::TwoBallAutoA(Intake * m_intake, Magazine * m_magazine, SwerveDriveTrain * m_swerve, Gyro * m_gyro, Vision * m_vision, DualShooter * m_dualshooter) {
 ...
 frc::Timer* m_timer = new frc::Timer;
  ...
  AddCommands(
    ...
    MotionProfile(m_swerve, m_gyro, coordinate{1.189_m, -0.34_m, -23.0_deg}),
    frc2::InstantCommand([m_intake] {m_intake->setPercentOutput(0.6);}),
    frc2::ParallelRaceGroup(MotionProfile(m_swerve, m_gyro, coordinate{2.05_m, -0.73_m, -33.56_deg})),
    ...
    frc2::InstantCommand([]{std::cout << "TwoBallAutoA Finished\n";})
  );
}
